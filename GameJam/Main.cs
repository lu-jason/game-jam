...
public partial class Main : Node2D {
	...
	public override void _Process(double delta) {
		...
		if (canMorph) { // ! Mainly here for conditions where player is not allowed to morph (idk what that is yet) - Josh
			...
			if (Input.IsActionPressed("morph_witch") && morphTimer > morphTimout) {
				GD.Print("morphing into witch"); // 1 + num1
				morphTimer = 0;
			}
			if (Input.IsActionPressed("morph_fox") && morphTimer > morphTimout) {
				GD.Print("morphing into fox"); // 2 + num2
				morphTimer = 0;
			}
			if (Input.IsActionPressed("morph_salamander") && morphTimer > morphTimout) {
				GD.Print("morphing into salamander"); // 3 + num3
				morphTimer = 0;
			}
			if (Input.IsActionPressed("morph_gargoyle") && morphTimer > morphTimout) {
